...
void Heap<T, PComparator>::trickleDown(unsigned int loc){
	//m = m;
	if (((m * loc) + 1) >= myHeap.size()){
    return;
  }
  unsigned int swappedChild = (m * loc) + 1;
	unsigned int smallestChild = swappedChild;
  for(unsigned int i = swappedChild + 1; i < swappedChild + m; i++){
    if (i < myHeap.size()){
      if (c(myHeap[i], myHeap[smallestChild])){
        smallestChild = i;
      }
    }
  }
	if (c(myHeap[smallestChild], myHeap[loc])){
